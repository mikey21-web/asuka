...
export default function MakeItYourself() {
    ...
    const [designResult, setDesignResult] = useState<{ reply: string, summary: string | null, imgPrompt: string | null } | null>(null)
    ...
    const handleGenerateDesign = async () => {
        ...
        try {
            const prompt = `BESPOKE DESIGN REQUEST:
Occasion: ${occasion}
Vibe: ${vibe}
Colors: ${colors}
Fit: ${fit}
Details: ${lapel}, ${buttons}, ${embroidery}, ${lining}
Initials: ${monogram}
PERSONAL VISION: ${personalInputs}

Please provide a detailed design summary and an image prompt for this specific vision.`
            ...
            if (data.reply) {
                setDesignResult({
                    reply: data.reply,
                    summary: data.design_summary,
                    imgPrompt: data.image_prompt
                })
                // Also update the visualization if an image prompt was returned
...
- Desired Fit: ${fit}
- Silhouette: ${selectedLook?.name || 'Custom'}
- Detailings: ${lapel}, ${buttons} accents, ${embroidery} embroidery, ${lining} lining.
- Custom Monogram: ${monogram || 'Standard'}

PERSONAL VISION:
${personalInputs || 'Fresh heritage design.'}

---------------------------------------------
AI DESIGN NOTES:
${designResult?.reply || 'Pending manual artisan review.'}
                            ...
                            <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
                                ...
                                <div className="grid md:grid-cols-2 gap-16 mb-16">
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="md:col-span-2">
                                            <label className={labelClasses}>Personal Vision / Special Requests</label>
                                            <textarea
                                                className="w-full border border-gray-100 p-4 bg-[#fafafa] rounded-sm outline-none focus:border-[#a17a58] transition-colors text-sm font-light min-h-[120px] resize-none"
                                                placeholder="Describe your dream outfit in your own words... e.g. 'I want a modern sherwani with geometric patterns inspired by Art Deco architecture, but keeping the silhouette very traditional.'"
                                                value={personalInputs}
                                                onChange={e => setPersonalInputs(e.target.value)}
                                            />
                                        </div>
                                ...
                                <div className="flex flex-col items-center gap-6">
                                    <button
                                        ...
                                        className="w-full max-w-md bg-white border-2 border-[#1a1410] text-[#1a1410] py-5 text-sm uppercase tracking-[4px] hover:bg-[#1a1410] hover:text-white transition-all shadow-xl font-bold rounded-sm flex items-center justify-center gap-3 disabled:opacity-50"
                                    >
                                        {designLoading ? 'Generating Design...' : 'Generate My Design ✦'}
                                    </button>
