...
import { groqChatWithFallback, DESIGN_SYSTEM_PROMPT } from '@/lib/groq'
import { checkRateLimit } from '@/lib/rate-limit'
import { validateDesignRequest } from '@/lib/validations'
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. Input Validation
    const validation = validateDesignRequest(rawBody);
    if (!validation.success) {
      return NextResponse.json({ error: validation.error }, { status: 400 });
    }

    const { message, session_id: rawSessionId } = validation.data!;
    ...
    const fullReply = await groqChatWithFallback(messages, 650, 0.8)
